...
N_CLUSTERS = 7  # Tune this

# === Load and group dataset by job ===
df = pd.read_csv(INPUT_CSV)

# Drop education and cluster info
...
# Average skill presence per job title
job_skill_matrix = df.groupby("label")[skill_cols].mean()

# Optional: scale (though binary presence usually doesn’t need this)
X = job_skill_matrix.values

# === Clustering using cosine similarity ===
# KMeans with cosine similarity: normalize vectors first
